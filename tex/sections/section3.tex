...
In both cases, balancing prices, $\lambda_{h}^{b}$, are sampled from days where up-regulation happened 0 $\ldots$ 24 hours with equal probability. In this way, days where up-regulation happened are essentially up-sampled. The reason for this approach is that the model learns more when up-regulation happens than when it does not.
